  ...
  $(function() {
    ...
    window.models.Query = Backbone.Model.extend({
      ...
      getGoogleNews: function(start, done) {
        ...
        cc("calling gnews");
        self = _this;
        query = _this.get("title");
        start || (start = 0);
        return $.get(_this.external_url, {
          source: 'google',
          q: query.toLowerCase(),
          start: start
        }, function(stories) {
          ...
          return this.getGoogleNews(start, done);
        }).fail(function() {
      ...
      getYahooNews: function(start, done) {
        var query, self;
        query = '"' + _this.get("title").toLowerCase() + '"';
        start || (start = 0);
        self = _this;
        $.get(_this.external_url, {
          source: 'yahoo',
          q: query,
          start: start
        }, function(stories) {
          ...
          total = 10;
          _.each(stories, this.addStory);
          if (start <= total) {
            this.getYahooNews(start + 50, done);
          } else if (done != null) {
            done(0, null);
          }
          return this;
